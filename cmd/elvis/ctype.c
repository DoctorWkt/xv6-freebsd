	...
	uchar	*flipcase;	/* list of non-standard lower/upper letter pairs */
{
	...
	{
		_ct_ctypes[(int)*scan] |= _CT_DIGIT | _CT_ALNUM;
	}

	/* add the whitespace */
	for (scan = (uchar *)" \t\n\r\f"; *scan; scan++)
	{
		_ct_ctypes[(int)*scan] |= _CT_SPACE;
	}

	/* add the standard ASCII letters */
	for (scan = (uchar *)"aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ"; *scan; scan += 2)
	{
		_ct_ctypes[(int)scan[0]] |= _CT_LOWER | _CT_ALNUM;
		_ct_ctypes[(int)scan[1]] |= _CT_UPPER | _CT_ALNUM;
		_ct_toupper[(int)scan[0]] = scan[1];
		_ct_tolower[(int)scan[1]] = scan[0];
	}
